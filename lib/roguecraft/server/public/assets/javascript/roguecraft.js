...
function create() {
  ...
  ws.onmessage = function(m) { 
	...
	explored = _.uniq(explored, false, function(a) { return a[0] * map.width + a[1]; });

	//console.log("obscuring");
	graphics.clear();
	map.forEach(function(tile) { 
	  var tilePosition = [tile.x, tile.y];
	  if (message.visible.containsArray(tilePosition)) {  //(_.contains(explored, tilePosition)) {
	    graphics.beginFill(0xFFFF0B, 0.1);
	  } else if (explored.containsArray(tilePosition)) {
	    graphics.beginFill(0x0A0AFF, 0.4);
	  } else {
