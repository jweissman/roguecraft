module Roguecraft
  # TODO try just to use minotaur's class structure here if possible... would simplify things
  # class Dungeon
  #   DEFAULT_WIDTH = 80
  #   DEFAULT_HEIGHT = 50
  ...
  #   def up_stairs
  #     row_with_up_stairs = @map.detect { |row| row.any? { |tile| tile.stair? && tile.up? } }
  #     row_with_up_stairs.detect { |tile| tile.stair? && tile.up? }
  #   end

  #   def down_stairs
  #     row_with_down_stairs = @map.detect { |row| row.any? { |tile| tile.stair? && tile.down? } }
  #     row_with_down_stairs.detect { |tile| tile.stair? && tile.down? }
  #   end
  ...
  #     @model  = Minotaur::Dungeon.new(width: @width, height: @height, room_count: 1)
